...
class User(UserMixin, db.Model): # Створення моделі користувача, яка наслідує UserMixin для інтеграції з Flask-Login
    ...
    categories = db.relationship('Categories', backref='user', lazy=True) # Встановлення зв'язку один-до-багатьох між користувачем та категоріями, з можливістю доступу до категорій через атрибут 'categories' та зворотного доступу до користувача через атрибут 'user' у моделі Categories
