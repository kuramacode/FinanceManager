...
def _json_safe(value):
    """Converts nested values into JSON-safe primitives."""
...
@login_required
def get_category():
    """Returns the current user's categories."""
    try:
        user_id = get_userid()
...
def create_category():
    ...
    try:
        ...
        emoji = (data.get("emoji") or "\U0001F6D2").strip()
...
def create_account():
    ...
    try:
        ...
        name = (data.get("name") or "").strip()
        initial_balance = float(data.get("initial_balance", data.get("balance", 0)) or 0)
        status = (data.get("status") or "active").strip()
        currency_code = (data.get("currency_code") or "UAH").strip()
        emoji = (data.get("emoji") or "\U0001F4B8").strip()
        type_ = (data.get("type") or "card").strip()
        ...
        response = account_service.create_account(
            user_id,
            name,
            initial_balance,
            status,
...
def update_account(account_id):
    ...
    try:
        ...
        response = account_service.update_account(
            account_id,
            user_id,
            name,
            initial_balance,
...
@login_required
def get_budgets():
    """Returns the current user's budgets."""
    try:
        user_id = get_userid()
