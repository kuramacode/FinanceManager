...
def get_transactions(userId):
    ...
    cur.execute(f'SELECT * FROM transactions WHERE user_id = ?', (userId,))
...
def transactions(): # Функція-обробник для відображення сторінки транзакцій користувача
    ...
    return render_template('transactions.html', transactions=transactions)# Відображає шаблон 'transactions.html' для користувача
